// app/api/transcription/cancel/[jobId]/route.ts
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ jobId: string }> }) {
  ...
  try {
    ...
    if (!backend) return formatErrorResponse('Could not connect to backend to cancel job.', 503);

    const resp = await fetch(`${backend}/api/transcription/cancel/${jobId}`, {
      method: 'POST',
