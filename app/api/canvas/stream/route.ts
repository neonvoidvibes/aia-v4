...
export async function POST(req: NextRequest) {
  console.log("[API /api/canvas/stream] Received POST request");
  const supabase = await createServerActionClient();

  try {
    const { data: { user }, error: authError } = await supabase.auth.getUser();
    if (authError || !user) {
      console.warn("[API /api/canvas/stream] Unauthorized request:", authError?.message);
    ...
    if (!activeBackendUrl) {
      return formatErrorResponse("Could not connect to backend for canvas streaming.", 503);
    }

    const targetUrl = `${activeBackendUrl}/api/canvas/stream`;
    console.log(`[API /api/canvas/stream] Forwarding POST to Python backend: ${targetUrl}`);
    ...
    if (session?.access_token) {
      ...
      console.error("[API /api/canvas/stream] Critical: Server-side session valid but access token missing.");
    ...
    console.log(`[API /api/canvas/stream] Backend response status: ${backendResponse.status}`);

    if (!backendResponse.ok) {
      const errorText = await backendResponse.text();
      console.error(`[API /api/canvas/stream] Python backend error: ${backendResponse.status}`, errorText);
      return formatErrorResponse(errorText || `Backend error (${backendResponse.status})`, backendResponse.status);
    }

    // Stream the response back to the client
    console.log("[API /api/canvas/stream] Streaming response from backend to client");
    ...
    console.error("[API /api/canvas/stream] Error in POST handler:", error);
    let message = error.message || 'An internal server error occurred during canvas streaming.';
    let statusCode = 500;

    // Handle timeout errors
    if (error.name === 'AbortError') {
      message = "Canvas stream request timed out.";
      statusCode = 504;
    }
    // Handle network errors
    else if (error.name === 'TypeError' && error.message.includes('NetworkError')) {
      message = "Network error connecting to the canvas streaming service.";
      statusCode = 503;
    }
    // Handle connection errors
    else if (error.code === 'ECONNREFUSED' || error.code === 'ENOTFOUND') {
      message = "Unable to connect to canvas streaming service. Please try again later.";
      statusCode = 503;
    }
