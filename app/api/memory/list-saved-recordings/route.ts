...
export async function GET(req: NextRequest) {
  console.log("[Memory List Recordings] Received GET request");
  ...
  try {
    ...
    if (!activeBackendUrl) {
      return formatErrorResponse("Could not connect to backend to list recording memories.", 503);
    }

    // Check if backend has an endpoint for listing saved recordings
    // If not, we'll need to check Pinecone directly or create a new endpoint
    const targetUrl = `${activeBackendUrl}/api/memory/list-saved-recordings?agent=${encodeURIComponent(agentName)}`;
    console.log(`[Memory List Recordings] Forwarding GET to ${targetUrl}`);
    ...
    const backendResponse = await fetchWithBackoff(targetUrl, { method: 'GET', headers: backendHeaders });
    
    const responseData = await backendResponse.json().catch(() => ({}));
    if (!backendResponse.ok) {
        // If the endpoint doesn't exist, return empty list for now
        if (backendResponse.status === 404) {
          console.warn("[Memory List Recordings] Backend endpoint not implemented yet, returning empty list");
          return NextResponse.json({ saved_recordings: {} }, { status: 200 });
        }
    ...
    console.error("[Memory List Recordings] Error in GET handler:", error);
